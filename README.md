...
| `secondaryShape` | `LiquidGlassShape` | — | **Android only** — a second glass body, smooth-min merged with `shape` so the two cling and fuse as they approach. Both share the primary's view-box, so author them as `path`/`points` shapes positioned within one box; two bounds-filling primitives merge concentrically. No-op on iOS. |
| `shapeSmoothing` | `number` (dp) | `0` | **Android only** — blend radius for the `secondaryShape` merge. `0` is a hard union with a visible crease; larger values pull a wider, softer neck and start bridging from further apart. `16`–`48` reads well for bodies ~100dp across. No-op on iOS. |
