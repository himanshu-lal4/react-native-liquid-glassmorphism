...
| `secondaryShape` | `LiquidGlassShape` | — | both | A second body, smooth-min merged with `shape`. Shares the primary's view-box. Android merges the distance fields natively; iOS gets a JS-computed merged outline. |
| `shapeSmoothing` | `number` (dp) | `0` | both | Blend radius. `0` = hard union. Bridges when surfaces are within ~`k/4` of the midpoint, so use ~`2 × gap`. |
