...
Title options:
1. Faking Apple's Liquid Glass on Android with a real-time AGSL shader
2. Android has no Liquid Glass — so I refracted the backdrop myself
3. Building Liquid Glass optics on Android: backdrop capture, RenderEffect, and one AGSL shader
4. How I reproduced iOS 26 Liquid Glass on Android in React Native
5. A glass lens, not a blur: reverse-engineering Liquid Glass for Android

Description:
Apple ships Liquid Glass as a system material on iOS 26. Android ships nothing like it — so I rebuilt the optics from scratch with backdrop capture, a RenderEffect blur, and a hand-written AGSL refraction shader. This is the engineering story of how each piece works and why it exists.

dev.to tags: reactnative, android, graphics, shaders
(alt tags: kotlin, agsl, rendereffect, gamedev)
-->

# Android has no Liquid Glass. Here's how I faked the optics convincingly, in real time.
