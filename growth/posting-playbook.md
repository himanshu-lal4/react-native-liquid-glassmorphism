...
- **B — one looping demo GIF** (used in the LinkedIn post, the README, and the article).
...
Then reshape the same A + B across every channel (LinkedIn = article's key points + the GIF; dev.to/Hashnode/Medium = syndicated article; README = the HN landing surface).
...
| Day | Action |
|---|---|
| 0 | Publish canonical article + demo GIF on your docs site. Polish the README: social-preview image + all GitHub topic tags (this is what ranked comparable projects #1 on Google). Post to the relevant directories (reactnative.directory ✅ done, LibHunt). |
| 1 | LinkedIn post (article's key points + the GIF). Share in Reactiflux / Expo Discord (short, non-spammy). |
| 3 | Show HN (glass) morning ET — link the repo, reply fast for the first hour. |
| 8–12 | Syndicate the article → dev.to (`canonical_url`) → Hashnode ("Add Original URL") → Medium (import), staggered a day apart. Pitch the newsletters (RN Newsletter form; tag @sebastienlorber for This Week in React; editor@cooperpress.com). |
...
Sequencing: run the **full glass wave first**, start the **tour-guide wave ~3–6 weeks later**. Between waves keep a light build-in-public trickle on LinkedIn so your presence stays warm.
...
- Disclose authorship on every HN / Stack Overflow post.
